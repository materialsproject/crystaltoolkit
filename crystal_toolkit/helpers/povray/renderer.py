...
class POVRayRenderer:
    ...
    _TEMPLATES = {path.stem: path.read_text() for path in (MODULE_PATH / "helpers" / "povray" / "templates").glob("*")}
    _ENV = Environment()

    @staticmethod
    def call_povray(povray_args: tuple[str] = ("render.ini", ), povray_path: str = SETTINGS.POVRAY_PATH):
        """
    ...
    def write_povray_input_scene_and_settings(
        ...
        render_settings = POVRayRenderer._ENV.from_string(POVRayRenderer._TEMPLATES["render"]).render(
            filename=scene_filename, image_filename=image_filename
        )
    ...
    def primitive_to_povray(obj: Primitive) -> str:
        ...
        if isinstance(obj, Spheres):
            ...
            return POVRayRenderer._ENV.from_string(POVRayRenderer._TEMPLATES["sphere"]).render(positions=positions,
                    radius=obj.radius,
                    color=color)
            ...
            color = POVRayRenderer._format_color_to_povray(obj.color)
            return POVRayRenderer._ENV.from_string(POVRayRenderer._TEMPLATES["cylinder"]).render(posPairs=position_pairs, color=color)
            ...
            return POVRayRenderer._ENV.from_string(POVRayRenderer._TEMPLATES["line"]).render(posPairs=position_pairs, cylCaps=cylCaps)

        elif isinstance(obj, Primitive):
            warn(f"Skipping {type(obj)}, not yet implemented. Submit PR to add support.")
