...
class StructureToSimple3DScene:
    ...
    def preprocess_structure(structure):
        ...
        if isinstance(structure, Structure):
            ...
            if not structure.is_ordered:
                ...
                # TODO: fix in NearNeighbor
                bonding_strategy = 'CutOffDictNN'


    def get_sites_to_draw(self):

        sg = StructureGraph()

        draw_sites = {(n, (0, 0, 0)) for n in range(len(sg.structure))}
        draw_bonds = set()
        draw_polyhedra = set()

        for idx, site in sg.structure:
            connected_sites = sg.get_connected_sites(idx)
            connected_site_set = {(cs.index, cs.jimage) for cs in connected_sites}
            draw_sites += connected_site_set
            draw_bonds



    @staticmethod
    def get_geometric_primitives(draw_sites):

        spheres = defaultdict(list)  # keyed by sphere type (radius, color, phi_start, phi_end)
        cylinders = defaultdict(list)  # keyed by sphere type
